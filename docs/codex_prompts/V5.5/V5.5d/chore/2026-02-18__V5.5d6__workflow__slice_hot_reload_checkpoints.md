...
- Commands (simple CLI):
  ...
  - `finalize <slice_id>`: dry run only (prints what would be deleted; no deletion).
  - `finalize <slice_id> --delete`: actually deletes `.slice_tmp/<slice_id>`.
  - Containment requirement: deletion must resolve inside `.slice_tmp/` root; traversal and absolute paths must be rejected.
...
- Creates scratch directory, writes notes, writes gate file.
- Finalize dry run keeps session intact.
- Finalize `--delete` removes only intended session.
- Path traversal/absolute IDs are rejected and cannot delete outside sandbox.
...
### 2026-02-18T00:00:00Z — D6 safety refinement
- Changed finalize behavior to be non-destructive by default:
  - `finalize <slice_id>` now performs dry run only.
  - `finalize <slice_id> --delete` performs actual deletion.
- Added containment safety contract:
  - reject traversal/absolute `slice_id`
  - enforce deletion target remains under `.slice_tmp/<slice_id>/`.
- Added deterministic tests for:
  - dry-run vs delete behavior
  - containment rejection cases (`..`, `../..`, `..\\..`, `C:\\Windows`).
- Reason:
  - align helper behavior with intended workflow safety contract and prevent accidental deletion.
