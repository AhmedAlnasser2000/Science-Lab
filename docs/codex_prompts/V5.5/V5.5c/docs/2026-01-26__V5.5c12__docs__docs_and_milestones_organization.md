# Codex Prompt — V5.5c docs + milestones organization pass (docs taxonomy + codex_prompts slice/type folders)
...
```bash
git add -A
git commit -m "chore(V5.5c) docs taxonomy cleanup and indexes"
git push
```
...
```bash
git add -A
git commit -m "chore(V5.5c) reorganize codex prompts by slice and type"
git push
```
...
```bash
git add -A
git commit -m "chore(V5.5c) add docs index link verification"
git push
```
